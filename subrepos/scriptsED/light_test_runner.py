...
class Runner:
	...
	def computeStats(self, studyBaseDir):
		...
		if os.path.exists(simulationLog):
			...
			if not stats.has_key("variables"):
				stats["variables"] = 0
				stats["constraints"] = 0
				stats["non-zero"] = 0
...
def globalReport(study, stats, status, accumulator):
	...
	entry["Statut"] = status
	for version in ["reference", "new"]:
		solve_time_by_run = stats[version]["solve_time_by_run"] if stats[version].has_key("solve_time_by_run") else {}
		entry["Temps total toutes résolutions %s" % version] = "%.2f" % sum(solve_time_by_run.values())
		entry["Indicateur mémoire %s" % version] = "%.2f" % stats[version]["memory_indicator"] if  stats[version].has_key("solve_time_by_run") else ""
		key_spx = filter(lambda x : x[0] == "0", solve_time_by_run.keys())
		key_pne = filter(lambda x : x[0] == "1", solve_time_by_run.keys())
...
def runAndCompare(configFile):
	...
	logging.info("	OK    : %d" % len(filter(lambda x:x == "OK", statusList)))
	logging.info("	KO    : %d" % len(filter(lambda x:x == "KO", statusList)))
	logging.info("	ERROR : %d" % len(filter(lambda x:x == "ERROR", statusList)))
